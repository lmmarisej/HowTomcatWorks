...
final class HttpProcessor
     ...
     */
    synchronized void assign(Socket socket) {   // 由connector将socket推送过来
        ...
        this.socket = socket;   // 通知processor处理新的socket
        available = true;
        notifyAll();
    ...
    private synchronized Socket await() {
        ...
        while (!available) {    // 防止假唤醒，强制调用者等待当前处理完成
            try {
                wait();
        ...
        Socket socket = this.socket;        // 使用局部变量，方便继续接收下一个请求
        available = false;  // 告诉当前processor线程，可以处理下一个请求了
        notifyAll();
    ...
    private void parseHeaders(SocketInputStream input)
        ...
        while (true) {
            ...
            if (header.equals(DefaultHeaders.AUTHORIZATION_NAME)) {     // 将读取到的header与标准名称对比
    ...
    private void process(Socket socket) {
        ...
        while (!stopped && ok && keepAlive) {
            ...
            try {
                ...
                if (ok) {
                    connector.getContainer().invoke(request, response);     // 交给容器，容器交给servlet来进行业务处理
                }
            } catch (ServletException e) {
     ...
     *
     * processor开启后，会一直循环处理来自连接器的传来的Socket，负责解析为HTTP，传给容器处理业务
     */
    public void run() {
